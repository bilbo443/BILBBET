(async function(){
  ...
  try {
    ...
    document.getElementById('app').innerHTML = `
      <div style="padding:2rem 1rem;max-width:420px;margin:0 auto;color:#f5f5f5;font-family:system-ui,sans-serif;">
        ...
        <p style="color:#9a9a9a;font-size:16px;">One of the files in <code>/data</code> failed to load (${esc(e.message)}). If you're setting this up from a fresh copy, check every file in the <code>data/</code> folder was uploaded, including any newly added ones -- a single missing or renamed data file stops the whole page from starting.</p>
      </div>`;
    return;
  }
  ...
  function renderPointProjection(round){
    ...
    return `<div class="bb-card" style="margin-bottom:1rem;">
      <div style="display:flex;align-items:baseline;gap:8px;flex-wrap:wrap;">
        <span style="font-size:14px;color:#9a9a9a;text-transform:uppercase;letter-spacing:0.05em;">Round ${round} projected score</span>
        <span style="font-size:20px;font-weight:800;color:#ffdd00;">${baseline.toFixed(1)} pts</span>
      </div>
      <div style="font-size:14px;color:#9a9a9a;margin-top:2px;">${note}</div>
    </div>`;
  }
  ...
  function renderHomeTab(){
    ...
    const fixtureCards = fixtures.length ? fixtures.map(p => `
      <div class="bb-card" style="display:flex;align-items:stretch;gap:12px;margin-bottom:8px;">
        <div style="flex:1;min-width:0;display:flex;flex-direction:column;justify-content:center;gap:3px;">
          <div style="font-size:13px;color:#9a9a9a;">${p.isCup ? esc(p.stage) + ' \u2014 ' + esc(p.division) : esc(p.division.replace(' (D1)',''))}</div>
          <div style="font-weight:600;display:flex;align-items:center;gap:6px;">${teamLogo(p.team,18)}${esc(p.team)}</div>
          <div style="font-weight:600;display:flex;align-items:center;gap:6px;color:#9a9a9a;font-size:15px;">vs ${esc(p.opp)}</div>
        </div>
        <div style="width:88px;flex-shrink:0;display:flex;flex-direction:column;justify-content:center;gap:2px;">
          <div style="font-size:12px;color:#6a6a6a;text-decoration:line-through;text-align:center;">${p.baseOdds.toFixed(2)}</div>
    ...
    const futureCards = futures.length ? futures.map(p => `
      <div class="bb-card" style="display:flex;align-items:stretch;gap:12px;margin-bottom:8px;">
        <div style="flex:1;min-width:0;display:flex;flex-direction:column;justify-content:center;gap:3px;">
          <div style="font-size:13px;color:#9a9a9a;">${esc(p.market)} \u2014 ${esc(String(p.division).replace(' (D1)',''))}</div>
          <div style="font-weight:600;display:flex;align-items:center;gap:6px;">${teamLogo(p.team,18)}${esc(p.team)}</div>
        </div>
        <div style="width:88px;flex-shrink:0;display:flex;flex-direction:column;justify-content:center;gap:2px;">
          <div style="font-size:12px;color:#6a6a6a;text-decoration:line-through;text-align:center;">${p.baseOdds.toFixed(2)}</div>
    ...
    const bestValueCard = bvw ? `
      <div class="bb-card" style="display:flex;align-items:center;gap:10px;">
        ...
        <div>
          ...
          <div style="font-size:14px;color:#9a9a9a;">Round ${bvw.round} &middot; ${esc(bvw.division.replace(' (D1)',''))} &middot; would have paid <span style="color:#ffdd00;font-weight:600;">${formatOdds(bvw.odds)}</span> \u2014 whether anyone backed it or not</div>
        </div>
    ...
    if(bb === null){
      ...
      <div class="bb-card" style="display:flex;align-items:center;gap:10px;">
        ...
        <div>
          ...
          <div style="font-size:14px;color:#9a9a9a;">${bb.selections.length} selection${bb.selections.length>1?'s':''} &middot; ${fmt(bb.stake)} staked &middot; returned <span style="color:#ffdd00;font-weight:600;">${fmt(bb.potentialReturn)}</span> clams</div>
        </div>
      </div>`;
    }
    ...
    return `
      ...
      <div class="bb-card" style="background:linear-gradient(135deg,#2a2410,#1a1a1a);border-color:#4a3a10;margin-bottom:16px;text-align:center;padding:1.25rem;">
        <div style="font-size:14px;letter-spacing:0.08em;color:#ffdd00;text-transform:uppercase;font-weight:700;">This week's boosted odds</div>
        <div style="font-size:14px;color:#9a9a9a;margin-top:4px;">Every pick below is +${Math.round((FEATURED_BOOST_MULTIPLIER-1)*100)}% on the normal price \u2014 just for being featured.</div>
      </div>
    ...
    return `<div style="max-width:480px;margin:0 auto;padding:14px;">
      ...
      <div style="text-align:center;margin-top:4px;">
        <a href="https://bilbo443.github.io/BILBBET/" target="_blank" rel="noopener noreferrer"
           style="display:inline-block;background:#ffdd00;color:#1a1a1a;font-weight:700;font-size:14px;padding:9px 18px;border-radius:4px;text-decoration:none;letter-spacing:0.02em;">
  ...
  function helpTip(id, text){
    ...
    return `<span style="position:relative;display:inline-block;">
        <span data-helptip="${esc(id)}" style="cursor:pointer;color:#9a9a9a;font-size:12px;border:1px solid #5a5a5a;border-radius:50%;width:14px;height:14px;display:inline-flex;align-items:center;justify-content:center;margin-left:4px;vertical-align:middle;">?</span>
        ${open ? `<span data-helptip-panel style="position:absolute;top:18px;left:0;z-index:70;background:#2a2a2a;border:1px solid #3d3d3d;border-radius:6px;padding:8px 10px;font-size:13px;font-weight:400;color:#cfcfcf;width:200px;box-shadow:0 4px 10px rgba(0,0,0,0.4);">${esc(text)}</span>` : ''}
      </span>`;
  }
  ...
  function teamSearchInput(id, currentValue, placeholder){
    ...
    return `<div style="position:relative;">
      ...
      <div id="${listId}" class="bb-team-dropdown" style="display:none;position:absolute;top:100%;left:0;right:0;max-height:200px;overflow-y:auto;background:#2a2a2a;border:1px solid #3d3d3d;border-radius:6px;z-index:60;margin-top:2px;box-shadow:0 4px 10px rgba(0,0,0,0.4);">
        ${ALL_TEAMS.map(t => `<div class="bb-team-option" data-team-option="${esc(t)}" data-option-for="${id}" style="padding:8px 12px;cursor:pointer;font-size:15px;">${esc(t)}</div>`).join('')}
      </div>
    </div>`;
  }
  ...
  function renderStorageWarning(){
    ...
    return `<div style="background:#3a2a26;color:#f0b8a8;padding:10px 14px;text-align:center;font-size:15px;border-bottom:2px solid #a3402f;">
  ...
  function renderTestingPhaseDisclaimer(){
    ...
    return `<div style="background:#2a2410;color:#e0d090;padding:10px 14px;text-align:center;font-size:15px;border-bottom:2px solid #4a3a10;">
  ...
  function renderTrashTalkBanner(){
    ...
    return `<div style="background:#2a1010;color:#e0a0a0;padding:10px 14px;text-align:center;font-size:15px;border-bottom:2px solid #4a1a1a;display:flex;justify-content:center;align-items:center;gap:10px;">
      <span>${esc(state.trashTalkBanner)}</span>
      <span id="dismiss-trash-talk" style="cursor:pointer;opacity:0.7;font-size:17px;line-height:1;">&times;</span>
    </div>`;
  }
  ...
  function renderFooter(){
    ...
    return `<div style="text-align:center;padding:24px 0 12px;">
      <span id="open-tos-footer" style="font-size:14px;color:#9a9a9a;text-decoration:underline;cursor:pointer;">Terms &amp; Conditions</span>
      <span style="color:#5a5a5a;margin:0 8px;">&middot;</span>
      <span id="open-readme-footer" class="${flashClass}" style="font-size:14px;color:#9a9a9a;text-decoration:underline;cursor:pointer;">Read me</span>
      <span style="color:#5a5a5a;margin:0 8px;">&middot;</span>
      <span id="open-tutorial-btn" style="font-size:14px;color:#9a9a9a;text-decoration:underline;cursor:pointer;" title="New here? Take the tour">Tutorial</span>
      <span style="color:#5a5a5a;margin:0 8px;">&middot;</span>
      <span id="open-contact-us-btn" style="font-size:14px;color:#9a9a9a;text-decoration:underline;cursor:pointer;">Contact us</span>
      <span style="color:#5a5a5a;margin:0 8px;">&middot;</span>
      <span id="open-install-app-footer" style="font-size:14px;color:#9a9a9a;text-decoration:underline;cursor:pointer;">Install app</span>
    </div>`;
  }
    ...
    return `
      <div id="tos-modal-backdrop" style="position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:110;display:flex;align-items:center;justify-content:center;padding:1rem;">
        <div class="bb-card" style="max-width:420px;width:100%;max-height:80vh;display:flex;flex-direction:column;">
          ...
          <div id="tos-scroll-content" style="overflow-y:auto;flex:1;border:1px solid #3d3d3d;border-radius:8px;padding:14px;margin-bottom:12px;font-size:16px;line-height:1.6;">
            ...
            <p style="color:#8a8a8a;font-size:14px;margin-bottom:0;">(End of terms.)</p>
          </div>
          ${registerMode ? `
            <label style="display:flex;align-items:flex-start;gap:8px;font-size:15px;margin-bottom:12px;">
  ...
  function renderFormModal(){
    ...
    if(thisSeasonRows.length){
      ...
      body = `<div style="font-size:14px;color:#9a9a9a;margin-bottom:8px;">This season, ${thisSeasonRows.length} round${thisSeasonRows.length!==1?'s':''} played \u2014 average ${avg.toFixed(1)}.</div>
        <div style="max-height:320px;overflow-y:auto;">
          ${thisSeasonRows.slice().reverse().map(r => `<div style="display:flex;justify-content:space-between;padding:7px 4px;border-bottom:1px solid #3d3d3d;font-size:15px;">
      ...
      if(hist && hist.length){
        ...
        body = `<div style="font-size:14px;color:#9a9a9a;margin-bottom:8px;">No results yet this season. Showing a sample of historical scores instead (${hist.length} total on record, average ${avg.toFixed(1)}) \u2014 not necessarily in chronological order, just the most recent real data available until this season's results come in.</div>
          <div style="max-height:320px;overflow-y:auto;display:flex;flex-wrap:wrap;gap:6px;">
            ${hist.slice(0, 20).map(s => `<span style="background:#2a2a2a;border-radius:4px;padding:4px 8px;font-size:14px;">${Math.round(s)}</span>`).join('')}
          </div>`;
      } else {
        body = `<p style="color:#9a9a9a;font-size:15px;">No results on record for this team yet.</p>`;
      }
    }
    return `
      <div id="form-modal-backdrop" style="position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:110;display:flex;align-items:center;justify-content:center;padding:1rem;">
        <div class="bb-card" style="max-width:380px;width:100%;">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
            ...
            <span style="font-size:14px;color:#9a9a9a;cursor:pointer;" id="close-form-modal-x">&times;</span>
          </div>
          ${body}
        </div>
      </div>`;
    ...
    return `
      <div id="welcome-modal-backdrop" style="position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:110;display:flex;align-items:center;justify-content:center;padding:1rem;">
        <div class="bb-card" style="max-width:380px;width:100%;text-align:center;">
          ...
          <p style="font-size:16px;line-height:1.6;">You're approved and ready to go \u2014 ${fmt(state.user.balance)} clams${carry > 0 ? ` are already in your account, including ${fmt(carry)} carried over from last season` : ' are already in your account'}.</p>
          <p style="font-size:15px;color:#9a9a9a;line-height:1.6;">New to Bilbbet? The <span id="welcome-open-tutorial" style="text-decoration:underline;cursor:pointer;color:#ffdd00;">tutorial</span> walks through what each tab does.</p>
    ...
    return `
      <div id="tutorial-modal-backdrop" style="position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:110;display:flex;align-items:center;justify-content:center;padding:1rem;">
        <div class="bb-card" style="max-width:420px;width:100%;">
          ${state.info ? `<div style="color:#7fbf8f;font-size:15px;margin-bottom:12px;padding-bottom:12px;border-bottom:1px solid #3d3d3d;">${esc(state.info)}</div>` : ''}
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;">
            <span style="font-size:14px;color:#9a9a9a;">Step ${i+1} of ${steps.length}</span>
            <span style="font-size:14px;color:#9a9a9a;cursor:pointer;" id="close-tutorial-modal-x">&times;</span>
          </div>
          <h3 style="margin:4px 0 10px;">${esc(step.title)}</h3>
          <p style="font-size:16px;line-height:1.6;margin:0 0 16px;">${esc(step.body)}</p>
      ...
      <div id="contact-us-modal-backdrop" style="position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:110;display:flex;align-items:center;justify-content:center;padding:1rem;">
        <div class="bb-card" style="max-width:420px;width:100%;">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
            ...
            <span style="font-size:14px;color:#9a9a9a;cursor:pointer;" id="close-contact-us-modal-x">&times;</span>
          </div>
          <p style="font-size:15px;line-height:1.6;color:#cfcfcf;margin:0 0 14px;">We understand not everything will go to expectations. To help us improve the site, we welcome your feedback \u2014 pick whatever's closest to how you're feeling below.</p>
          ...
          ${isOther ? `<textarea id="feedback-other-text" placeholder="Go on, let it out." style="width:100%;min-height:90px;background:#2a2a2a;border:1px solid #3d3d3d;border-radius:6px;color:#eee;padding:8px;font-size:15px;box-sizing:border-box;margin-bottom:10px;">${esc(state.feedbackOtherText)}</textarea>` : ''}
    ...
    return `
      <div id="readme-modal-backdrop" style="position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:110;display:flex;align-items:center;justify-content:center;padding:1rem;">
        <div class="bb-card" style="max-width:420px;width:100%;max-height:80vh;display:flex;flex-direction:column;">
          ...
          <div style="overflow-y:auto;flex:1;border:1px solid #3d3d3d;border-radius:8px;padding:14px;margin-bottom:12px;font-size:16px;line-height:1.6;">
    ...
    return `
      <div id="install-app-modal-backdrop" style="position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:110;display:flex;align-items:center;justify-content:center;padding:1rem;">
        <div class="bb-card" style="max-width:420px;width:100%;max-height:80vh;display:flex;flex-direction:column;">
          ...
          <div style="overflow-y:auto;flex:1;font-size:16px;line-height:1.6;">
      ...
      <div id="login-modal-backdrop" style="position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:100;display:flex;align-items:center;justify-content:center;padding:1rem;">
        <div style="max-width:340px;width:100%;">
          <div style="text-align:center;margin-bottom:1rem;">
            ...
            <p style="color:#9a9a9a;font-size:16px;margin:4px 0 0;">Log in to place a bet</p>
          </div>
          <form id="login-form" class="bb-card" style="display:flex;flex-direction:column;gap:10px;">
            <div><span style="font-size:14px;color:#9a9a9a;display:block;margin-bottom:4px;">${state.customNameMode ? 'Your display name' : 'Your Eliza team'}</span>
              ...
              ${state.registeringMode ? `<span id="toggle-custom-name" style="display:block;font-size:13px;color:#9a9a9a;text-decoration:underline;cursor:pointer;margin-top:4px;">${state.customNameMode ? 'Actually, I have a team in the Eliza Cup' : "Not part of the Eliza Cup? Make up your own name"}</span>` : ''}
              <button type="button" class="bb-btn ghost" id="use-admin-login" style="margin-top:6px;width:100%;font-size:14px;padding:6px;">${state.adminLoginMode ? '\u2713 Logging in as admin' : 'Log in as admin instead'}</button>
            </div>
            <div><span style="font-size:14px;color:#9a9a9a;display:block;margin-bottom:4px;">PIN</span>
              <input class="bb-input" id="f-pin" type="password" inputmode="numeric" value="${esc(state.pin)}"/></div>
            ${state.registeringMode ? `
              <label style="display:flex;align-items:flex-start;gap:8px;font-size:15px;">
            ...
            ${state.error ? `<div style="color:#c0604f;font-size:15px;">${esc(state.error)}</div>` : ''}
            ${state.info ? `<div style="color:#7fbf8f;font-size:15px;">${esc(state.info)}</div>` : ''}
            ${state.registeringMode ? `
          ...
          <p style="font-size:14px;color:#9a9a9a;text-align:center;margin-top:1rem;">Everyone starts with 1,000 clams once an admin approves your registration.</p>
          ${(!supabaseClient && !hasRealStorage) ? `<p style="font-size:14px;color:#c0604f;text-align:center;margin-top:0.5rem;">Running without persistent storage &mdash; set up Supabase (see supabase/schema.sql) or open inside Claude's artifact panel for accounts to be saved between visits.</p>` : ''}
        </div>
      </div>`;
  }

  function header(){
    const brand = `<span style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">${siteLogoBadge(32)}<strong style="letter-spacing:1px;font-size:22px;text-transform:uppercase;">BILBBET</strong><a href="https://elizacup.com/" target="_blank" rel="noopener noreferrer" style="color:#9a9a9a;font-size:14px;text-decoration:none;border-left:1px solid #3d3d3d;padding-left:10px;margin-left:2px;">Official Fantasy Partner of the Eliza Cup</a></span>`;
    if(!state.user){
      return `
        <div style="display:flex;align-items:center;justify-content:space-between;padding:14px 4px;border-bottom:5px solid var(--bb-accent);margin-bottom:1rem;">
          ...
          <div style="display:flex;align-items:center;gap:10px;">
            <button class="bb-btn ghost" id="open-team-search-btn" style="padding:6px 12px;font-size:15px;">Find a team</button>
    ...
    return `
      <div style="display:flex;align-items:center;justify-content:space-between;padding:14px 4px;border-bottom:5px solid var(--bb-accent);margin-bottom:1rem;flex-wrap:wrap;gap:8px;">
        ${brand}
        <div style="display:flex;align-items:center;gap:14px;font-size:16px;">
          <button class="bb-btn ghost" id="open-team-search-btn" style="padding:6px 12px;font-size:15px;">Find a team</button>
          <span>${fmt(state.user.balance)} clams</span>
          <span style="color:#9a9a9a;">${esc(state.user.username)}${(state.user.isAdmin && adminNeedsAttention()) ? ' <span title="Needs attention" style="font-size:14px;">\u{1F6A9}</span>' : ''}</span>
    ...
    return '<div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:10px;">' +
      currentTabs().map(t => {
        ...
        const adminFlag = (t==='ADMIN' && state.user && state.user.isAdmin && adminNeedsAttention())
          ? ' <span title="Needs attention" style="font-size:13px;">\u{1F6A9}</span>' : '';
        const tipFlag = (t==='TIPPING' && state.tipReminderStatus === true)
          ? ' <span title="You haven\'t submitted your tips for this week yet" style="font-size:13px;">\u{1F6A9}</span>' : '';
  ...
  function futuresMarketTabs(){
    return '<div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:14px;">' +
      Object.entries(FUTURES.market_labels).filter(([key]) =>
        ...
        `<div class="bb-tab ${state.futureMarketTab===key?'active':''}" data-marketkey="${key}" style="font-size:14px;padding:6px 10px;">${esc(label)}</div>`
      ).join('') +
      `<div class="bb-tab ${state.futureMarketTab==='leading_at'?'active':''}" data-marketkey="leading_at" style="font-size:14px;padding:6px 10px;">To Be Leading At&hellip;</div>` +
      '</div>';
  }
  ...
  function renderLeadingAtMarket(scopeKey){
    ...
    const picker = `<div class="bb-card" style="margin-bottom:1rem;display:flex;align-items:center;gap:10px;">
        <span style="font-size:14px;color:#9a9a9a;">Round</span>
        <select class="bb-select" id="leadingat-round" style="width:170px;">${roundOptions(state.leadingAtRound)}</select>
      </div>
      <p style="color:#9a9a9a;font-size:14px;margin-bottom:10px;">Who's on top of the table after this specific round, not who wins the season.</p>`;
  ...
  function cupMarketTabs(labelsKey){
    ...
    return '<div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:10px;">' +
      `<div class="bb-tab ${state.futureMarketTab==='fixtures'?'active':''}" data-marketkey="fixtures" style="font-size:14px;padding:6px 10px;">Current fixtures</div>` +
      (isEcl ? `<div class="bb-tab ${state.futureMarketTab==='groups'?'active':''}" data-marketkey="groups" style="font-size:14px;padding:6px 10px;">Groups</div>` : '') +
  ...
  function renderCupFixtures(compKey){
    ...
    const calendarNote = roundInfo
      ? `<p style="color:#9a9a9a;font-size:14px;margin-bottom:10px;">Round ${state.currentRound}, per the 26/27 calendar${roundInfo.overridden?' (admin override)':''}: <strong style="color:#ffdd00;">${esc(roundInfo.stage)}</strong>.</p>`
      : `<p style="color:#9a9a9a;font-size:14px;margin-bottom:10px;">Round ${state.currentRound} isn't a scheduled ${esc(compKey)} round on the 26/27 calendar.</p>`;
    if(!fixtures.length){
    ...
    return calendarNote + '<div class="bb-card" style="padding:0;overflow:hidden;">' +
      fixtures.map((f,i) => `<div data-cupfixture="${esc(compKey)}|${i}" style="display:flex;justify-content:space-between;align-items:center;padding:12px 16px;cursor:pointer;${i<fixtures.length-1?'border-bottom:1px solid #3d3d3d;':''}">
        ...
        <span class="bb-btn ghost" style="padding:5px 12px;font-size:14px;">View market</span>
      </div>`).join('') + '</div>' +
      `<p style="color:#9a9a9a;font-size:14px;margin-top:10px;">Odds here use the same head-to-head model as the division matches -- cup-specific pricing (rewarding spike ability for the FA Cup, tough-opposition form for the ECL) isn't wired in yet.</p>`;
  }
  ...
  function playoffSubTabBar(){
    return '<div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:14px;">' +
      PLAYOFF_DIVS.map(d => `<div class="bb-tab ${state.playoffSubTab===d?'active '+divColorClass(d):''}" data-playoffsubtab="${esc(d)}" style="font-size:14px;padding:6px 10px;">${d.replace('DIVISION ','Div ')}</div>`).join('') +
      '</div>';
  }
  ...
  function renderPlayoffsTab(){
    ...
    const windowNote = inPlayoffWindow
      ? `<p style="color:#9a9a9a;font-size:14px;margin-bottom:10px;">Round ${state.currentRound} is a scheduled playoff week for ${esc(div)}.</p>`
      : `<p style="color:#9a9a9a;font-size:14px;margin-bottom:10px;">Round ${state.currentRound} isn't one of ${esc(div)}'s playoff weeks (those are Rounds 24-26).</p>`;
    if(!fixtures.length){
  ...
  function categoryPauseControl(category, label){
    ...
    return `<label style="display:flex;align-items:center;gap:6px;font-size:14px;color:#9a9a9a;margin-bottom:8px;" title="Pause this whole market for everyone (admin only)">
  ...
  function renderH2HMarket(m){
    ...
    function row(id, label, pct, oddsInfo){
      if(oddsInfo.suspended){
        return `<div class="bb-outcome" style="opacity:0.5;cursor:default;">
          <span>${esc(label)} <span style="color:#9a9a9a;font-size:14px;">(${pct.toFixed(1)}%)</span></span>
    ...
    return `<div class="bb-card" style="margin-bottom:1rem;">
      ...
      <div style="display:flex;justify-content:space-between;margin-bottom:12px;">
        <div><div style="font-size:14px;color:#9a9a9a;">${esc(m.teamA)} viable range</div><div style="font-size:20px;font-weight:600;">${m.aRange[0]}&ndash;${m.aRange[1]}</div></div>
        <div style="text-align:right;"><div style="font-size:14px;color:#9a9a9a;">${esc(m.teamB)} viable range</div><div style="font-size:20px;font-weight:600;">${m.bRange[0]}&ndash;${m.bRange[1]}</div></div>
      </div>
      ${(() => {
        const found = getH2HRecord(m.teamA, m.teamB);
        if(!found) return `<p style="color:#9a9a9a;font-size:14px;margin-bottom:12px;">\u{1F195} First time these two have played each other \u2014 no history to show.</p>`;
        const r = found.rec;
        const aWins = found.flipped ? r.aLosses : r.aWins;
        const aLosses = found.flipped ? r.aWins : r.aLosses;
        return `<div style="background:#1b1b1b;border:1px solid #3d3d3d;border-radius:8px;padding:10px 12px;margin-bottom:12px;">
          <div style="font-size:14px;color:#9a9a9a;margin-bottom:2px;">All-time head-to-head &mdash; played ${r.played}</div>
          <div style="font-size:16px;">${esc(m.teamA)} <strong>${aWins}</strong> &ndash; <strong>${r.draws}D</strong> &ndash; <strong>${aLosses}</strong> ${esc(m.teamB)}</div>
          <div style="font-size:13px;color:#8a8a8a;margin-top:4px;">Most recent meeting: ${esc(r.lastMatch)}</div>
          ${m.edge && m.edge.applied ? `<div style="font-size:13px;color:#ffdd00;margin-top:6px;">\u26A1 ${esc(m.edge.favored)} carries a${m.edge.decay<1?' reduced':' slight'} edge here from a considerably lopsided head-to-head record (${m.edge.winMargin>0?aWins:aLosses}W-${m.edge.winMargin>0?aLosses:aWins}L across ${r.played} meetings)${m.edge.decay<1?`, weighted down since the last meeting was a while ago`:''}.</div>` : ''}
        </div>`;
      })()}
      <h4 style="margin:0 0 8px;font-size:15px;color:#9a9a9a;">Match result</h4>
      ...
      <h4 style="margin:14px 0 8px;font-size:15px;color:#9a9a9a;">Handicap${helpTip('handicap', 'A virtual head start or deficit applied to level the odds \u2014 pick a team to "cover the line," meaning win by more than (or lose by less than) the handicap.')}</h4>
  ...
  function h2hSubTabBar(){
    const renderItem = t => `<div class="bb-tab ${state.h2hSubTab===t?'active':''}" data-h2hsubtab="${esc(t)}" style="font-size:14px;padding:6px 10px;display:flex;align-items:center;gap:4px;">${subTabLogo(t)}${t==='CUSTOM MATCHUP'?'Custom matchup':(t==='PLAYOFFS'?'Playoffs':t.replace(' (D1)',''))}</div>`;
  ...
  function futuresSubTabBar(){
    const renderItem = t => `<div class="bb-tab ${state.futuresSubTab===t?'active '+divColorClass(t):''}" data-futuressubtab="${esc(t)}" style="font-size:14px;padding:6px 10px;display:flex;align-items:center;gap:4px;">${subTabLogo(t)}${t==='RODDY'?'The Roddy':t.replace(' (D1)','')}</div>`;
  ...
  function quickOddsButton(pickId, label, teamLabel, oddsInfo){
    if(oddsInfo.suspended){
      return `<span class="bb-btn ghost" style="padding:6px 10px;font-size:14px;opacity:0.5;cursor:default;display:inline-flex;align-items:center;gap:6px;">${teamLogo(teamLabel,16)}${esc(teamLabel)} susp.</span>`;
    }
    const selected = state.slip.some(s=>s.id===pickId);
    return `<button class="bb-btn ${selected?'':'ghost'}" data-pick="${esc(pickId)}" data-label="${esc(label)}" data-odds="${oddsInfo.odds}" style="padding:6px 10px;font-size:14px;display:inline-flex;align-items:center;gap:6px;">${teamLogo(teamLabel,16)}${esc(teamLabel)} ${formatOdds(oddsInfo.odds)}</button>`;
  }
  ...
  function priceOnlyButton(pickId, label, oddsInfo){
    if(oddsInfo.suspended){
      return `<span class="bb-btn ghost" style="padding:8px 14px;font-size:15px;opacity:0.5;cursor:default;width:100%;text-align:center;">susp.</span>`;
    }
    const selected = state.slip.some(s=>s.id===pickId);
    return `<button class="bb-btn ${selected?'':'ghost'}" data-pick="${esc(pickId)}" data-label="${esc(label)}" data-odds="${oddsInfo.odds}" style="padding:8px 14px;font-size:15px;font-weight:700;width:100%;text-align:center;">${formatOdds(oddsInfo.odds)}</button>`;
  }
  ...
  function renderTippingTab(){
    ...
    const subTabs = `<div style="display:flex;gap:6px;margin-bottom:10px;flex-wrap:wrap;">
        <div class="bb-tab ${state.tippingSubTab==='PICKS'?'active':''}" data-tippingtab="PICKS" style="font-size:14px;padding:6px 10px;display:flex;align-items:center;gap:5px;">This week's tips${state.tipReminderStatus===true ? ' <span title="You haven\'t submitted your tips for this week yet" style="font-size:13px;">\u{1F6A9}</span>' : ''}</div>
        <div class="bb-tab ${state.tippingSubTab==='PRESEASON'?'active':''}" data-tippingtab="PRESEASON" style="font-size:14px;padding:6px 10px;">Pre-season</div>
        <div class="bb-tab ${state.tippingSubTab==='LEADERBOARD'?'active':''}" data-tippingtab="LEADERBOARD" style="font-size:14px;padding:6px 10px;">Leaderboard</div>
        <div class="bb-tab ${state.tippingSubTab==='PRIZES'?'active':''}" data-tippingtab="PRIZES" style="font-size:14px;padding:6px 10px;">Prizes</div>
      </div>`;
    const intro = `<p style="color:#9a9a9a;font-size:14px;margin-bottom:10px;">Free to play, but topping it pays real clams \u2014 see the Prizes tab for the full breakdown. Correct tips score two ways: a straight tally, and what a 1-clam bet on that tip would have paid (upsets are worth more). A drawn fixture pays half credit either way \u2014 half the tally, half the odds \u2014 and never counts against a perfect round. Nothing saves until you hit Confirm, and you can come back and change your tips right up until this round locks.</p>` +
      (FIXTURES_ARE_PLACEHOLDER ? `<div class="bb-card" style="background:#3a3320;margin-bottom:10px;padding:10px 12px;font-size:14px;color:#e0d090;">\u26A0\uFE0F The weekly schedule shown here is a placeholder, not yet the real season draw \u2014 specific matchups may still change once the real draw is confirmed. Everything you tip still counts as normal; this notice will come down once the schedule is final.</div>` : '');
    ...
    const roundBrowser = lastPlayedForBrowse >= 1 ? `<div class="bb-card" style="margin-bottom:1rem;display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
        <span style="font-size:14px;color:#9a9a9a;">Viewing</span>
    ...
    const renderSectionItem = s => `<div class="bb-tab ${state.tippingSection===s.key?'active':''}" data-tipping-section="${s.key}" style="font-size:14px;padding:6px 10px;">${esc(s.label)}</div>`;
    ...
    const mrMedianNote = activeSection.divs.some(d => isMrMedianWeek(d, round))
      ? `<div class="bb-card" style="margin-bottom:1rem;padding:12px;font-size:14px;line-height:1.6;color:#cfcfcf;">
    ...
    if(locked){
      const rewardBanner = state.tippingRewardBanner ? `<div class="bb-card" style="margin-bottom:1rem;background:#3a3320;display:flex;justify-content:space-between;align-items:center;">
          <span style="font-size:15px;">${esc(state.tippingRewardBanner)}</span>
      ...
      const readOnly = Object.entries(byDiv).map(([div, picks]) => `<div class="bb-card" style="margin-bottom:1rem;">
          <strong style="font-size:15px;">${esc(div.replace(' (D1)',''))}</strong>
          <div style="margin-top:6px;">${picks.map(p => `<div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid #333333;font-size:15px;">
      ...
      const perfectBadge = (state.user && !state.user.isAdmin && state.perfectRoundStatus[state.user.username.toLowerCase() + '|' + round + '|' + activeSection.key])
        ? `<div class="bb-card" style="margin-bottom:1rem;background:#2a3a20;text-align:center;padding:10px;font-size:15px;color:#8fc98f;">\u2705 Perfect round in ${esc(activeSection.label)} for Round ${round}!</div>`
        : '';
    ...
    if(!divsWithFixtures.length){
      const confirmBar = `<div class="bb-card" style="display:flex;justify-content:space-between;align-items:center;gap:10px;margin-bottom:1rem;">
          <span style="font-size:14px;color:#9a9a9a;">${pendingCount} tip${pendingCount!==1?'s':''} selected across all sections${dirty?' \u2014 not yet saved':''}</span>
    ...
    const sections = divsWithFixtures.map(div => {
      ...
      const rows = fixtures.map(([teamA, teamB], i) => {
        ...
        if(teamB === 'MR MEDIAN'){
          ...
          return `<div style="padding:4px 0;">
              <label style="display:flex;align-items:center;gap:8px;padding:6px 10px;border-radius:6px;background:${checked?'#3a3320':'#2a2a2a'};cursor:pointer;font-size:14px;">
        ...
        const radio = (team, oddsInfo, side) => {
          if(oddsInfo.suspended) return `<span style="flex:1;text-align:center;font-size:14px;color:#9a9a9a;opacity:0.6;">susp.</span>`;
          const checked = current && current.team === team;
          return `<label style="flex:1;display:flex;align-items:center;gap:6px;padding:6px 10px;border-radius:6px;background:${checked?'#3a3320':'#2a2a2a'};cursor:pointer;font-size:14px;">
        ...
        return `<div style="display:flex;gap:8px;align-items:center;padding:6px 0;border-bottom:1px solid #333333;">
          ${radio(teamA, aOdds, 'home')}<span style="color:#9a9a9a;font-size:13px;">v</span>${radio(teamB, bOdds, 'away')}
        </div>`;
      }).join('');
      ...
      return `<div class="bb-card" style="margin-bottom:1rem;">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
          <strong style="font-size:15px;">${esc(div.replace(' (D1)',''))}</strong>
          <span style="font-size:14px;color:#9a9a9a;">${doneStored}/${fixtures.length} confirmed</span>
        </div>
        ${rows}
      </div>`;
    }).join('');
    ...
    const sectionMultiCard = sectionStatus.allConfirmed ? `<div class="bb-card" style="margin-bottom:1rem;background:#3a3320;">
        <div style="display:flex;justify-content:space-between;align-items:center;">
          <span style="font-size:15px;">\u{1F3AF} Perfect round for ${esc(activeSection.label)} \u2014 ${sectionStatus.confirmedCount} legs</span>
  ...
  function renderEntireFieldOption(round){
    ...
    return `<div class="bb-card" style="margin-top:1rem;background:#22301f;">
        <div style="display:flex;justify-content:space-between;align-items:center;">
          <span style="font-size:15px;">\u{1F30D} Entire field \u2014 ${fieldStatus.confirmedCount} confirmed tip${fieldStatus.confirmedCount!==1?'s':''} across everything</span>
  ...
  function renderPreseasonTab(){
    ...
    if(locked){
      const rows = PRESEASON_SLOTS.map(slot => {
        ...
        return `<div style="padding:8px 0;border-bottom:1px solid #333333;">
            <div style="font-size:14px;color:#9a9a9a;margin-bottom:4px;">${esc(slot.label)}</div>
            <div style="display:flex;flex-wrap:wrap;gap:8px;">
              ${picks.map(p => `<span style="display:flex;align-items:center;gap:5px;background:#2a2a2a;border-radius:6px;padding:4px 10px;font-size:15px;">${teamLogo(p.team,16)}${esc(p.team)} <span style="color:#ffdd00;font-weight:600;">${formatOdds(p.odds)}</span></span>`).join('')}
            </div>
          </div>`;
      }).filter(Boolean);
      const ownPicks = rows.length
        ? `<div class="bb-card" style="margin-bottom:1rem;"><strong style="font-size:15px;">Your picks</strong>${rows.join('')}</div>`
    ...
    const group = (title, slots) => `<div class="bb-card" style="margin-bottom:1rem;">
        <strong style="font-size:15px;display:block;margin-bottom:8px;">${esc(title)}</strong>
        ${slots.map(slot => {
          const options = preseasonSlotOptions(slot);
          const current = state.preseasonPending[slot.key] || [];
          return `<div style="margin-bottom:12px;">
              <div style="font-size:14px;color:#9a9a9a;margin-bottom:6px;">${esc(slot.label)} ${slot.count>1?`<span style="color:#7fbf8f;">(${current.length}/${slot.count})</span>`:''}</div>
              <div style="display:flex;flex-wrap:wrap;gap:6px;">
                ${options.map(o => {
                  ...
                  return `<button class="bb-btn ${picked?'':'ghost'}" data-preseason-pick="${esc(slot.key)}|${esc(o.team)}|${displayOdds}|${slot.count}" style="padding:5px 10px;font-size:14px;display:flex;align-items:center;gap:5px;${o.suspended?'opacity:0.85;':''}">${teamLogo(o.team,14)}${esc(o.team)} ${formatOdds(displayOdds)}${o.suspended?' <span style="color:#9a9a9a;font-size:12px;" title="Heavy favorite -- no real betting price, so this pick pays a flat 1.00 (stake back) rather than real odds.">(favorite)</span>':''}</button>`;
                }).join('')}
              </div>
            </div>`;
        }).join('')}
    ...
    const confirmBar = `<div class="bb-card" style="position:sticky;bottom:0;display:flex;justify-content:space-between;align-items:center;gap:10px;margin-bottom:1rem;">
        <span style="font-size:14px;color:#9a9a9a;">${Object.values(state.preseasonPending).flat().length} pick${Object.values(state.preseasonPending).flat().length!==1?'s':''} selected${dirty?' \u2014 not yet saved':''}</span>
  ...
  function renderPreseasonAdminResults(){
    return `<div class="bb-card" style="margin-bottom:1rem;border:1px solid #ffdd00;">
        <strong style="font-size:15px;display:block;margin-bottom:8px;color:#ffdd00;">Admin: record final results</strong>
        <p style="font-size:13px;color:#9a9a9a;margin-bottom:10px;">What actually happened, not a prediction -- this is what every punter's pre-season picks get scored against.</p>
        ${PRESEASON_SLOTS.map(slot => {
          const options = preseasonSlotOptions(slot);
          const current = state.preseasonResults[slot.key] || [];
          return `<div style="margin-bottom:10px;">
              <div style="font-size:14px;color:#9a9a9a;margin-bottom:4px;">${esc(slot.label)} ${slot.count>1?`<span style="color:#7fbf8f;">(${current.length}/${slot.count})</span>`:''}</div>
              <div style="display:flex;flex-wrap:wrap;gap:5px;">
                ${options.map(o => {
                  const picked = current.includes(o.team);
                  return `<button class="bb-btn ${picked?'':'ghost'}" data-final-result="${esc(slot.key)}|${esc(o.team)}|${slot.count}" style="padding:4px 8px;font-size:13px;">${esc(o.team)}</button>`;
                }).join('')}
              </div>
            </div>`;
        }).join('')}
  ...
  function renderAllPreseasonPicks(){
    ...
    return `<div class="bb-card" style="margin-top:1rem;">
        <strong style="font-size:15px;">Everyone's picks</strong>
        <div style="margin-top:8px;">
          ${state.preseasonAllPicks.map(r => `<div style="display:flex;align-items:center;gap:8px;padding:8px 0;border-bottom:1px solid #333333;flex-wrap:wrap;">
              <span style="font-size:14px;width:110px;flex-shrink:0;">${esc(r.username)}</span>
  ...
  function prizeCard(title, amount, body, example){
    return `<div class="bb-card" style="margin-bottom:1rem;">
        <div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:6px;">
          ...
          <span style="color:#ffdd00;font-weight:700;font-size:16px;">${esc(amount)}</span>
        </div>
        <p style="font-size:14px;color:#cfcfcf;margin:0 0 8px;line-height:1.5;">${body}</p>
        <p style="font-size:13px;color:#9a9a9a;margin:0;font-style:italic;">${esc(example)}</p>
      </div>`;
  }
  function renderPrizesTab(){
    const description = `<p style="color:#9a9a9a;font-size:14px;margin-bottom:14px;">Every reward below pays real clams, straight into your balance the moment it's earned \u2014 check My Bets \u2192 Balance history for the full record. A dead heat splits the amount between everyone tied, rounded UP, so a 3-way tie for 10 clams pays 4 each, not 3.33.</p>` +
  ...
  function renderRecentWinners(){
    if(state.recentWinners === null){
      ...
      return `<p style="color:#9a9a9a;font-size:15px;">Loading&hellip;</p>`;
    }
    if(!state.recentWinners.length){
      return `<p style="color:#9a9a9a;font-size:15px;">No prizes claimed yet \u2014 check back once a round wraps up.</p>`;
    }
    const recent = state.recentWinners.slice(0, 30);
    return `<div class="bb-card">
        ${recent.map(w => `<div style="display:flex;justify-content:space-between;align-items:center;padding:7px 0;border-bottom:1px solid #333333;font-size:14px;">
  ...
  function renderTippingLeaderboard(){
    const kindBar = `<div style="display:flex;gap:4px;margin-bottom:1rem;">
        <div class="bb-tab ${state.leaderboardKind==='WEEKLY'?'active':''}" data-leaderboard-kind="WEEKLY" style="font-size:14px;padding:6px 10px;">Weekly tipping</div>
        <div class="bb-tab ${state.leaderboardKind==='PRESEASON'?'active':''}" data-leaderboard-kind="PRESEASON" style="font-size:14px;padding:6px 10px;">Pre-season</div>
      </div>`;
    if(state.leaderboardKind === 'PRESEASON'){
      return kindBar + renderPreseasonLeaderboard();
    }
    const rewardNote = `<p style="color:var(--bb-text-muted);font-size:14px;margin-bottom:10px;">Topping a leaderboard here pays real clams \u2014 see the Prizes tab for exactly how much and what qualifies.</p>`;
    const lastPlayed = state.currentRound - 1;
    const sectionTabs = `<div style="display:flex;gap:4px;flex-wrap:wrap;margin-bottom:10px;">
        ${LEADERBOARD_SECTIONS.map(s => `<div class="bb-tab ${state.tippingLeaderboardSection===s.key?'active':''}" data-leaderboard-section="${s.key}" style="font-size:14px;padding:6px 10px;">${esc(s.label)}</div>`).join('')}
      </div>`;
    if(lastPlayed < 1){
    ...
    const controls = `<div class="bb-card" style="margin-bottom:1rem;display:flex;flex-wrap:wrap;gap:10px;align-items:center;">
        <div style="display:flex;gap:4px;">
          <div class="bb-tab ${state.tippingLeaderboardMode==='WEEKLY'?'active':''}" data-tipping-lb-mode="WEEKLY" style="font-size:14px;padding:5px 10px;">Weekly</div>
          <div class="bb-tab ${state.tippingLeaderboardMode==='OVERALL'?'active':''}" data-tipping-lb-mode="OVERALL" style="font-size:14px;padding:5px 10px;">Overall</div>
        </div>
        <span style="font-size:14px;color:var(--bb-text-muted);">${state.tippingLeaderboardMode==='WEEKLY'?'Round':'Through round'}</span>
    ...
    if(state.user && !state.user.isAdmin){
      ...
      if(rows.length){
        yourStanding = `<div class="bb-card" style="margin-bottom:1rem;border-color:#4a4a2a;">
            <strong style="font-size:15px;color:#eee;">Your standing in this view</strong>
            <div style="margin-top:6px;">
              ${rows.map(([label, r]) => `<div style="display:flex;justify-content:space-between;padding:4px 0;font-size:14px;color:#cfcfcf;">
  ...
  function renderFixtureList(div){
    ...
    return '<div class="bb-card" style="padding:0;overflow:hidden;">' +
      markets.map((m, i) => {
        ...
        return `<div style="display:flex;align-items:stretch;gap:12px;padding:12px 16px;${i<markets.length-1?'border-bottom:1px solid #3d3d3d;':''}">
          <div style="flex:1;min-width:0;display:flex;flex-direction:column;justify-content:center;gap:3px;">
            ...
            <span class="bb-btn ghost" data-fixture-expand="${esc(div)}|${i}" style="align-self:flex-start;padding:3px 9px;font-size:12px;margin-top:2px;">Full market (draw &amp; handicap)</span>
          </div>
      ...
      '</div>' +
      '<p style="color:#9a9a9a;font-size:14px;margin-top:10px;">Fixture list is a projected double round-robin, not an official 26/27 schedule \u2014 swap in the real one once fixtures are confirmed. Tap either team\'s price to back the moneyline directly, or open the full market for the draw and handicap.</p>';
  }
    ...
    if(state.h2hSubTab === 'CUSTOM MATCHUP'){
      return stripe + roundBar + h2hSubTabBar() + `<div class="bb-card" style="margin-bottom:1rem;">
        <div style="display:flex;gap:10px;align-items:flex-end;flex-wrap:wrap;">
          <div style="flex:1;min-width:180px;"><span style="font-size:14px;color:#9a9a9a;display:block;margin-bottom:4px;">Team A</span>
            ${teamSearchInput('team-a', state.teamA)}</div>
          <div style="flex:1;min-width:180px;"><span style="font-size:14px;color:#9a9a9a;display:block;margin-bottom:4px;">Team B</span>
        ...
        ${state.teamA && state.teamB && state.teamA===state.teamB ? '<p style="color:#c0604f;font-size:15px;margin:8px 0 0;">Pick two different teams.</p>' : ''}
        ${(state.teamA && !ALL_TEAMS.includes(state.teamA)) || (state.teamB && !ALL_TEAMS.includes(state.teamB)) ? '<p style="color:#c0604f;font-size:15px;margin:8px 0 0;">No match for that team name \u2014 pick one from the suggestions as you type.</p>' : ''}
  ...
  function renderTxHistoryList(){
    if(state.txHistory === null){
      ...
      return `<p style="color:#9a9a9a;font-size:15px;margin-top:8px;">Loading&hellip;</p>`;
    }
    if(!state.txHistory.length){
      return `<p style="color:#9a9a9a;font-size:15px;margin-top:8px;">Nothing here yet.</p>`;
    }
    const recent = state.txHistory.slice(0, 30);
    return `<div style="margin-top:8px;">
        ${recent.map(tx => `<div style="display:flex;justify-content:space-between;align-items:center;padding:6px 0;border-bottom:1px solid #333333;font-size:14px;">
  ...
  function renderMyBetsTab(){
    ...
    const prefsCard = `<div class="bb-card" style="margin-bottom:1rem;">
        ...
        <label style="display:flex;align-items:flex-start;gap:8px;font-size:15px;cursor:pointer;">
    ...
    const txCard = `<div class="bb-card" style="margin-bottom:1rem;">
        <div data-toggle-tx-history style="display:flex;justify-content:space-between;align-items:center;cursor:pointer;">
          ...
          <span style="font-size:14px;color:#9a9a9a;">${state.txHistoryExpanded?'Hide \u25B4':'Show \u25BE'}</span>
        </div>
    ...
    const careerBox = (h && h.totalBets > 0) ? `
      <div class="bb-card" style="margin-bottom:1rem;">
        <div style="font-size:15px;font-weight:600;margin-bottom:8px;">Career record (carried over from previous seasons)</div>
        <div style="display:flex;gap:20px;flex-wrap:wrap;">
          <div><div style="font-size:14px;color:#9a9a9a;">Bets</div><div style="font-size:18px;font-weight:600;">${h.totalBets}</div></div>
          <div><div style="font-size:14px;color:#9a9a9a;">Won</div><div style="font-size:18px;font-weight:600;color:#4a9166;">${h.winningBets}</div></div>
          <div><div style="font-size:14px;color:#9a9a9a;">Lost</div><div style="font-size:18px;font-weight:600;color:#a3402f;">${h.losingBets}</div></div>
          <div><div style="font-size:14px;color:#9a9a9a;">Void</div><div style="font-size:18px;font-weight:600;">${h.voidBets}</div></div>
          <div><div style="font-size:14px;color:#9a9a9a;">Net</div><div style="font-size:18px;font-weight:600;">${(h.winnings-h.losses)>=0?'+':''}${fmt(h.winnings-h.losses)}</div></div>
        </div>
      </div>` : '';
    const legacyBox = (state.user.legacyBestBets && state.user.legacyBestBets.length) ? `
      <div class="bb-card" style="margin-bottom:1rem;">
        <div style="font-size:15px;font-weight:600;margin-bottom:8px;">All-time best bets</div>
        ${state.user.legacyBestBets.map((b,i) => `
          <div style="padding:6px 0;${i<state.user.legacyBestBets.length-1?'border-bottom:1px solid #2a2a2a;':''}">
            <div style="font-size:15px;">${b.selections.map(s=>esc(s.label)).join(', ')}</div>
            <div style="font-size:13px;color:#8a8a8a;">Stake ${fmt(b.stake)} @ ${b.combinedOdds.toFixed(2)} &rarr; won ${fmt(b.potentialReturn)}${b.season?' &mdash; '+esc(b.season):''}</div>
          </div>`).join('')}
      </div>` : '';
    ...
    return prefsCard + txCard + careerBox + legacyBox + `
      <div class="bb-card" style="margin-bottom:1rem;display:flex;gap:20px;flex-wrap:wrap;">
        <div><div style="font-size:14px;color:#9a9a9a;">Pending</div><div style="font-size:18px;font-weight:600;">${pending}</div></div>
        <div><div style="font-size:14px;color:#9a9a9a;">Won</div><div style="font-size:18px;font-weight:600;color:#4a9166;">${won}</div></div>
        <div><div style="font-size:14px;color:#9a9a9a;">Lost</div><div style="font-size:18px;font-weight:600;color:#a3402f;">${lost}</div></div>
        <div><div style="font-size:14px;color:#9a9a9a;">Voided</div><div style="font-size:18px;font-weight:600;">${voided}</div></div>
        <div><div style="font-size:14px;color:#9a9a9a;">Net (settled bets)</div><div style="font-size:18px;font-weight:600;">${netFromSettled>=0?'+':''}${fmt(netFromSettled)}</div></div>
      </div>
      ...
      <p style="font-size:14px;color:#9a9a9a;margin-top:10px;">
  ...
  function renderTeamSearchResults(team){
    if(!team || !ALL_TEAMS.includes(team)){
      return '<p style="color:#9a9a9a;font-size:15px;">Type a team name and pick a suggestion.</p>';
    }
  ...
  function renderTeamSearchPanel(){
    ...
    return `<div class="bb-card" style="margin-bottom:1rem;">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
        <strong style="font-size:15px;">Find a team's markets</strong>
  ...
  function renderTeamDirectory(){
    ...
    return `<div class="bb-card" style="margin-bottom:1rem;">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
          <strong style="font-size:16px;">All teams</strong>
        ...
        ${!filtered.length ? '<p style="color:var(--bb-text-muted);font-size:15px;">No teams match that search.</p>' : ''}
        <div style="max-height:520px;overflow-y:auto;">
        ${letters.map(letter => `
          <div style="font-size:13px;font-weight:700;color:var(--bb-text-muted);text-transform:uppercase;letter-spacing:0.04em;padding:8px 4px 4px;">${esc(letter)}</div>
          ${groups[letter].map(t => `<div class="bb-tab" data-view-team-profile="${esc(t)}" style="display:flex;align-items:center;gap:8px;padding:8px 10px;border-radius:8px;cursor:pointer;">
            ${teamLogo(t, 22)}<span style="font-size:15px;">${esc(t)}</span>
            <span style="margin-left:auto;font-size:13px;color:var(--bb-text-muted);">${esc((findTeamDivision(t)||'').replace(' (D1)',''))}</span>
          </div>`).join('')}
        `).join('')}
        </div>
      </div>`;
  ...
  function renderTeamProfile(teamName){
    ...
    const tabs = `<div style="display:flex;gap:4px;flex-wrap:wrap;margin-bottom:1rem;">
        ${TEAM_PROFILE_SUBTABS.map(s => `<div class="bb-tab ${state.teamProfileSubTab===s.key?'active':''}" data-team-profile-subtab="${s.key}" style="font-size:14px;padding:6px 10px;">${esc(s.label)}</div>`).join('')}
      </div>`;
    const header = `<div class="bb-card" style="margin-bottom:1rem;display:flex;align-items:center;gap:12px;">
        <button class="bb-btn ghost" id="team-profile-back" style="padding:6px 10px;font-size:14px;">&larr;</button>
        ${teamLogo(teamName, 44)}
        <div>
          <div style="font-size:18px;font-weight:700;">${esc(teamName)}</div>
          <div style="font-size:14px;color:var(--bb-text-muted);">${esc(div.replace(' (D1)',''))}</div>
        </div>
      </div>`;
    let body;
  ...
  function renderTeamProfileOverview(teamName, div){
    ...
    const oddsHtml = `<div class="bb-card" style="margin-bottom:10px;">
        <strong style="font-size:15px;">${esc(div.replace(' (D1)',''))} betting markets</strong>
        <div style="margin-top:8px;">
          ${oddsBlock.map(([label, val]) => `<div style="display:flex;justify-content:space-between;padding:4px 0;font-size:15px;">
    ...
    const standingsHtml = lastPlayed < 1
      ? `<div class="bb-card"><p style="color:var(--bb-text-muted);margin:0;font-size:15px;">Season hasn't started yet \u2014 standings will appear once Round 1 is played.</p></div>`
      : (() => {
          ...
          return `<div class="bb-card">
              <div style="display:flex;gap:20px;flex-wrap:wrap;margin-bottom:14px;">
                <div><div style="font-size:13px;color:var(--bb-text-muted);text-transform:uppercase;">Position</div><div style="font-size:20px;font-weight:700;">${rank} <span style="font-size:14px;color:var(--bb-text-muted);font-weight:400;">of ${standings.length}</span></div></div>
                <div><div style="font-size:13px;color:var(--bb-text-muted);text-transform:uppercase;">Points</div><div style="font-size:20px;font-weight:700;">${row.points}</div></div>
                <div><div style="font-size:13px;color:var(--bb-text-muted);text-transform:uppercase;">Record</div><div style="font-size:20px;font-weight:700;">${row.won}-${row.drawn}-${row.lost}</div></div>
              </div>
              <div style="font-size:14px;color:var(--bb-text-muted);">Played ${row.played} &middot; Score for ${row.scoreFor} &middot; Score against ${row.scoreAgainst}</div>
            </div>`;
        })();
    return oddsHtml + standingsHtml +
      `<div style="margin-top:10px;"><span id="team-profile-view-markets" data-team="${esc(teamName)}" style="cursor:pointer;color:var(--bb-accent);font-size:14px;">View this team's betting markets &rarr;</span></div>`;
  }
    ...
    return `<div class="bb-card" style="padding:0;overflow:hidden;">
        <table style="width:100%;border-collapse:collapse;font-size:16px;">
          <thead><tr style="border-bottom:2px solid var(--bb-border-light);">
            <th style="text-align:left;padding:8px 14px;font-size:13px;color:var(--bb-text-muted);text-transform:uppercase;">Rd</th>
            <th style="text-align:left;padding:8px 8px;font-size:13px;color:var(--bb-text-muted);text-transform:uppercase;">Opponent</th>
            <th style="text-align:right;padding:8px 8px;font-size:13px;color:var(--bb-text-muted);text-transform:uppercase;">Score</th>
            <th style="text-align:center;padding:8px 14px;font-size:13px;color:var(--bb-text-muted);text-transform:uppercase;">Result</th>
          </tr></thead>
  ...
  function renderTeamProfileCompetitions(teamName, div){
    ...
    sections.push(`<div class="bb-card" style="margin-bottom:10px;">
        <strong style="font-size:15px;">FA Cup</strong>
        <p style="font-size:14px;color:var(--bb-text-muted);margin:4px 0 8px;">Every team enters the FA Cup. Round-by-round elimination tracking isn't available until real results start coming in \u2014 these are their current chances at each stage.</p>
        ${Object.entries(FA_CUP_LABELS_FOR_PROFILE).map(([key, label]) => faCupOdds[key] ? `<div style="display:flex;justify-content:space-between;padding:3px 0;font-size:15px;"><span style="color:var(--bb-text-muted);">${esc(label)}</span><span>${faCupOdds[key].suspended?'suspended':formatOdds(faCupOdds[key].odds)}</span></div>` : '').join('')}
      </div>`);
    if(inEcl){
      const eclOdds = {};
      ...
      sections.push(`<div class="bb-card">
          <strong style="font-size:15px;">ECL</strong>
          <p style="font-size:14px;color:var(--bb-text-muted);margin:4px 0 8px;">One of 12 teams in this season's ECL field.</p>
          ${Object.entries(ECL_LABELS_FOR_PROFILE).map(([key, label]) => eclOdds[key] ? `<div style="display:flex;justify-content:space-between;padding:3px 0;font-size:15px;"><span style="color:var(--bb-text-muted);">${esc(label)}</span><span>${eclOdds[key].suspended?'suspended':formatOdds(eclOdds[key].odds)}</span></div>` : '').join('')}
        </div>`);
    } else {
      sections.push(`<div class="bb-card"><p style="font-size:14px;color:var(--bb-text-muted);margin:0;">Not in this season's ECL field.</p></div>`);
    }
  ...
  function renderTeamProfileBilbbetHistory(teamName){
    ...
    if(!d.user){
      return `<div class="bb-card"><p style="color:var(--bb-text-muted);margin:0;font-size:15px;">No bilbbet account registered under "${esc(teamName)}" yet.</p></div>`;
    }
    return `<div class="bb-card" style="margin-bottom:10px;display:flex;gap:20px;flex-wrap:wrap;">
        <div><div style="font-size:13px;color:var(--bb-text-muted);text-transform:uppercase;">Clam balance</div><div style="font-size:20px;font-weight:700;">${fmt(d.user.balance)}</div></div>
        <div><div style="font-size:13px;color:var(--bb-text-muted);text-transform:uppercase;">Tips this season</div><div style="font-size:20px;font-weight:700;">${fmtCorrect(d.tipping.correct)}/${d.tipping.total}</div></div>
        <div><div style="font-size:13px;color:var(--bb-text-muted);text-transform:uppercase;">Odds points</div><div style="font-size:20px;font-weight:700;">${d.tipping.oddsPoints.toFixed(2)}</div></div>
      </div>
      <div class="bb-card" style="padding:0;overflow:hidden;">
        <div style="padding:12px 14px 6px;"><strong style="font-size:15px;">Recent bets</strong></div>
        ${!d.bets.length ? '<p style="color:var(--bb-text-muted);font-size:15px;padding:0 14px 14px;">No bets placed yet.</p>' :
          d.bets.slice(0,10).map((b,i) => `<div style="padding:8px 14px;${i<Math.min(d.bets.length,10)-1?'border-bottom:1px solid var(--bb-border);':''}font-size:15px;display:flex;justify-content:space-between;">
  ...
  function renderRoundExtremeMarket(kind){
    ...
    let html = `<div class="bb-card" style="margin-bottom:10px;">
      <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px;margin-bottom:8px;">
        <div style="font-size:15px;font-weight:600;">${label} ${round} (${sublabel})</div>
    ...
    if(isRoundBlocked(round)){
      const msg = round < state.currentRound
        ? '\u{1F512} This round has already been played &mdash; betting closed.'
        : '\u{1F512} Betting is closed for this round right now &mdash; hidden until it reopens.';
      html += `<p style="color:#9a9a9a;font-size:14px;">${msg}</p></div>`;
      return html;
    }
    ...
    if(selectedTeam){
      ...
      if(!o){
        html += `<p style="color:#9a9a9a;font-size:14px;margin-top:8px;">No match for that team name.</p>`;
      } else if(o.suspended){
    ...
    html += `<button class="bb-btn ghost" data-toggle-extreme-list="${kind}" style="margin-top:10px;font-size:14px;padding:6px 12px;">${expanded ? 'Hide full list \u25b4' : 'Show every team in odds order \u25be'}</button>`;
  ...
  function fixedSpecialDropdown(pickPrefix, marketLabel, outcomes, selectedTeam, selectId){
    ...
    if(selectedTeam){
      const o = outcomes.find(x => x.team === selectedTeam);
      if(!o){
        odds_row = `<p style="color:#9a9a9a;font-size:14px;margin-top:8px;">No match for that team name.</p>`;
      } else if(o.suspended){
    ...
    const expanded = state.specialsExtremeExpanded === kind;
    let listHtml = `<button class="bb-btn ghost" data-toggle-extreme-list="${kind}" style="margin-top:10px;font-size:14px;padding:6px 12px;">${expanded ? 'Hide full list \u25b4' : 'Show every team in odds order \u25be'}</button>`;
    if(expanded){
    ...
    return `<div class="bb-card" style="margin-bottom:10px;">
      <div style="font-size:15px;font-weight:600;margin-bottom:8px;">${esc(marketLabel)}</div>
  ...
  function specialsSubTabBar(){
    ...
    return '<div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:14px;">' +
      TABS.map(t => `<div class="bb-tab ${state.specialsSubTab===t.key?'active':''}" data-specialssubtab="${t.key}" style="font-size:14px;padding:6px 10px;">${t.label}</div>`).join('') +
      '</div>';
  }
  ...
  function renderSpecialsTab(){
    ...
    if(state.specialsSubTab === 'round'){
      html += `<p style="color:#9a9a9a;font-size:14px;margin-bottom:10px;">Pick any upcoming round to back who'll be leading or trailing after it.</p>`;
    ...
    if(state.specialsSubTab === 'season'){
      html += `<p style="color:#9a9a9a;font-size:14px;margin-bottom:10px;">These cover the whole season -- set once, resolved at the end of Round 26.</p>`;
    ...
    html += '<h3>Suggest your own</h3><div class="bb-card">' +
      ...
      `<p style="font-size:14px;color:#9a9a9a;margin:8px 0 0;">The admin reviews every idea and either sets a price and adds it above, or turns it down.</p>` +
      '</div>';
    return html;
  }
  ...
  function renderSortableLeaderboardTable(rows, sortByKey, sortDir, currentUsername, title, subtitle, showUpcomingCheck){
    ...
    const headerCell = c => {
      ...
      return `<th data-sort-col="${c.key}" style="text-align:right;padding:8px 8px;color:${active?'var(--bb-accent)':'var(--bb-text-muted)'};font-weight:600;font-size:13px;text-transform:uppercase;letter-spacing:0.04em;cursor:pointer;user-select:none;white-space:nowrap;">
    ...
    return `<div class="bb-card" style="padding:0;overflow:hidden;">
      <div style="padding:14px 18px 8px;display:flex;justify-content:space-between;align-items:baseline;flex-wrap:wrap;gap:4px;">
        <strong style="font-size:16px;">${esc(title)}</strong>
        ${subtitle ? `<span style="font-size:13px;color:var(--bb-text-muted);">${subtitle}</span>` : ''}
      </div>
      <div style="overflow-x:auto;">
      <table style="width:100%;border-collapse:collapse;font-size:16px;min-width:360px;">
        <thead>
          <tr style="border-bottom:2px solid var(--bb-border-light);">
            <th style="text-align:left;padding:8px 18px;color:var(--bb-text-muted);font-weight:600;font-size:13px;text-transform:uppercase;letter-spacing:0.04em;">#</th>
            <th style="text-align:left;padding:8px 8px;color:var(--bb-text-muted);font-weight:600;font-size:13px;text-transform:uppercase;letter-spacing:0.04em;">Punter</th>
        ...
        <tbody>
          ${sorted.map((t,i) => {
            ...
            return `<tr style="border-bottom:1px solid var(--bb-border);${rowBg}">
              ...
              <td style="padding:9px 8px;font-weight:${isYou?'700':'400'};${isYou?'color:var(--bb-accent);':''}">${esc(t.username)}${upcomingTick}${isYou?' <span style="font-size:12px;color:var(--bb-text-muted);font-weight:400;">(you)</span>':''}${!graded?' <span style="font-size:12px;color:var(--bb-text-muted);">(joined, no results yet)</span>':''}</td>
  ...
  function renderStatsTab(){
    ...
    function leaderboard(title, rows, valueFmt){
      if(!rows.length) return `<h4 style="color:#9a9a9a;">${title}</h4><p style="color:#9a9a9a;font-size:15px;">Nothing to show yet.</p>`;
    ...
    return '<h3 style="margin-top:0;">Site stats</h3>' +
      `<div class="bb-card" style="margin-bottom:1.25rem;display:flex;gap:20px;flex-wrap:wrap;">
        <div><div style="font-size:14px;color:#9a9a9a;">Total clams wagered</div><div style="font-size:18px;font-weight:600;">${fmt(s.totalWagered)}</div></div>
        <div><div style="font-size:14px;color:#9a9a9a;">Bets placed</div><div style="font-size:18px;font-weight:600;">${s.totalBets}</div></div>
        <div><div style="font-size:14px;color:#9a9a9a;">Punters</div><div style="font-size:18px;font-weight:600;">${s.totalPunters}</div></div>
      </div>` +
      ...
      (s.myKittyRank ? `<p style="color:#9a9a9a;font-size:14px;margin-top:-10px;margin-bottom:1.25rem;">You're ranked #${s.myKittyRank.rank} of ${s.myKittyRank.of} punters by balance.</p>` : '') +
      ...
      (s.lineBeat.ready
        ...
        : '<p style="color:#9a9a9a;font-size:15px;">Not enough completed rounds yet to make this meaningful \u2014 check back around Round 5.</p>');
  }
  ...
  function renderAdminTab(){
    ...
    const tabBar = '<div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:16px;">' +
      TABS.map(t => `<div class="bb-tab ${state.adminSubTab===t.key?'active':''}" data-adminsubtab="${t.key}" style="font-size:15px;padding:7px 12px;position:relative;">${t.label}${t.flag?' <span style="display:inline-block;width:7px;height:7px;border-radius:50%;background:#ffdd00;margin-left:4px;"></span>':''}</div>`).join('') +
      '</div>';

    const SEASON_HTML = `\n      <h3 style="margin-top:0;">Season progress</h3>
      <div class="bb-card" style="margin-bottom:1.5rem;">
        <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:8px;">
          <span style="font-size:15px;color:#9a9a9a;">Next round to be played:</span>
          <strong style="font-size:17px;">Round ${state.currentRound}</strong>
          ${state.currentRoundOverride
            ? `<span style="font-size:14px;color:#e0c060;">(manually overridden -- would otherwise be Round ${deriveCurrentRoundFromDate()} automatically)</span>`
            : `<span style="font-size:14px;color:#8fc98f;">(auto-derived from today's date, no admin action needed)</span>`}
        </div>
        <p style="font-size:14px;color:#9a9a9a;margin:0 0 10px;">
        ...
        <span style="font-size:14px;color:#9a9a9a;display:block;margin-top:8px;">Rounds before this are greyed out everywhere as already played. Changing this reopens betting fresh.</span>
      </div>
      <div class="bb-card" style="margin-bottom:1.5rem;display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
        <span style="font-size:15px;color:#9a9a9a;">Home tab's featured picks are computed once per round and then locked in -- if the selection logic changes, an already-computed round won't pick up the fix on its own.</span>
        ...
        ${state.refreshedFeaturedInfo ? `<span style="font-size:14px;color:#8fc98f;width:100%;">${esc(state.refreshedFeaturedInfo)}</span>` : ''}
      </div>
      <h3>End of season</h3>
      <div class="bb-card" style="margin-bottom:1.5rem;border-color:#a3402f;">
        <p style="font-size:14px;color:#9a9a9a;margin-top:0;">
        ...
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:12px;">
          <input type="text" id="season-label-override" placeholder="e.g. 26/27" value="${esc(state.currentSeasonLabel || '')}" style="width:100px;padding:6px 8px;font-size:15px;"/>
          <button class="bb-btn ghost" id="save-season-label-btn" style="padding:6px 12px;font-size:14px;">Correct current season</button>
          <span style="font-size:13px;color:#9a9a9a;">Only for fixing a wrong auto-derived value -- doesn't archive or reset anything, just relabels going forward.</span>
        </div>
      ...
      <div class="bb-card" style="margin-bottom:1.5rem;">
        ...
        ${['ELIZA','DIV2','DIV3'].map(key => {
          ...
          return `<label style="display:flex;align-items:center;gap:8px;font-size:15px;margin-bottom:8px;cursor:pointer;">
              <input type="checkbox" data-toggle-season-closed="${key}" ${state.seasonClosed[key]?'checked':''}/>
              <span>${esc(label)} season closed</span>
            </label>`;
        }).join('')}
        <label style="display:flex;align-items:center;gap:8px;font-size:15px;padding-top:6px;border-top:1px solid #333333;margin-top:4px;cursor:pointer;">
      ...
      <div class="bb-card" style="margin-bottom:1.5rem;">
        <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:8px;">
          ...
          <span style="font-size:15px;">Round ${state.currentRound} betting is currently ${state.roundBettingOpen?'open':`closed (${state.closeScope==='all'?'entire betting markets':'H2H only'})`}.</span>
        </div>
        <p style="font-size:14px;color:#9a9a9a;margin:0 0 10px;">
        ...
        <div style="margin-bottom:8px;">
          <span style="font-size:14px;color:#9a9a9a;display:block;margin-bottom:4px;">When closing, what should it cover?</span>
          <label style="display:inline-flex;align-items:center;gap:6px;font-size:15px;margin-right:16px;">
          ...
          <label style="display:inline-flex;align-items:center;gap:6px;font-size:15px;">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;">
          <button class="bb-btn ghost" id="close-betting-btn" ${!state.roundBettingOpen?'disabled':''} style="padding:8px 14px;font-size:14px;">Close betting now</button>
          <button class="bb-btn" id="reopen-betting-btn" ${state.roundBettingOpen?'disabled':''} style="padding:8px 14px;font-size:14px;">Reopen betting</button>
        </div>
        <p style="font-size:14px;color:#9a9a9a;margin:10px 0 6px;">"H2H only" affects this round's H2H, leading-at, and win/lose-the-round markets, leaving season-long futures bettable. "Entire betting markets" also locks division/Roddy/cup futures until reopened. Either way, the affected round's H2H fixture list is hidden (not just unclickable) while closed.</p>
      </div>
      <h3>Odds refresh</h3>
      <div class="bb-card" style="margin-bottom:1.5rem;">
        <p style="font-size:14px;color:#9a9a9a;margin-top:0;">
        ...
        ${state.oddsRefreshRequested
          ? `<div style="display:flex;align-items:center;gap:10px;"><span class="bb-pill" style="background:#4a3a10;color:#ffdd00;">Requested</span><button class="bb-btn ghost" id="clear-odds-refresh-btn" style="padding:6px 12px;font-size:14px;">Clear</button></div>`
          : `<button class="bb-btn ghost" id="request-odds-refresh-btn" style="padding:8px 14px;font-size:14px;">Flag odds as needing a refresh</button>`}
      </div>
`;
    const FIXTURES_HTML = `\n      <h3>Cup fixtures (FA Cup &amp; ECL)</h3>
      <div class="bb-card" style="margin-bottom:1.5rem;">
        <p style="font-size:14px;color:#9a9a9a;margin-top:0;">Based on the 26/27 calendar. Rounds may shift to dodge unplanned byes/doubles -- override the current round below if the calendar's out of date.</p>
        ${['FA CUP','ECL'].map(comp => {
          ...
          <div style="margin-bottom:14px;">
            <div style="font-size:15px;font-weight:600;margin-bottom:6px;">${esc(comp)}</div>
            <div style="font-size:14px;color:#9a9a9a;margin-bottom:6px;">
            ...
            <div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap;margin-bottom:10px;">
              ...
              <button class="bb-btn ghost" data-set-cupoverride="${esc(comp)}" style="padding:6px 12px;font-size:14px;">Force this round as cup round</button>
              <button class="bb-btn ghost" data-set-cupoverride-off="${esc(comp)}" style="padding:6px 12px;font-size:14px;">Force NOT a cup round</button>
              ${hasOverride ? `<button class="bb-btn ghost" data-clear-cupoverride="${esc(comp)}" style="padding:6px 12px;font-size:14px;">Clear override (use calendar)</button>` : ''}
            </div>
            ${(state.cupFixtures[comp]||[]).length ? (state.cupFixtures[comp]||[]).map((f,i) => `
              <div style="display:flex;justify-content:space-between;align-items:center;padding:6px 0;border-bottom:1px solid #2a2a2a;font-size:15px;">
                <span>${esc(f.teamA)} vs ${esc(f.teamB)}</span>
                <span data-remove-cupfixture="${esc(comp)}|${i}" style="cursor:pointer;color:#9a9a9a;font-size:14px;">remove</span>
              </div>`).join('') : `<p style="color:#9a9a9a;font-size:14px;">No fixtures set for this competition.</p>`}
            <div style="display:flex;gap:6px;align-items:flex-end;margin-top:8px;flex-wrap:wrap;">
              ...
              <button class="bb-btn" data-add-cupfixture="${esc(comp)}" style="padding:8px 14px;font-size:14px;">Add</button>
              ${(state.cupFixtures[comp]||[]).length ? `<button class="bb-btn ghost" data-clear-cupfixtures="${esc(comp)}" style="padding:8px 14px;font-size:14px;">Clear all</button>` : ''}
            </div>
          </div>`;
        }).join('')}
        <p style="font-size:14px;color:#9a9a9a;margin-top:4px;">Set fixtures when there's an actual cup weekend; clear them once it's passed so the tab correctly shows "No fixtures scheduled yet" the rest of the time.</p>
      </div>
      <h3>Playoff fixtures (Division 2 &amp; 3, Rounds 24-26)</h3>
      <div class="bb-card" style="margin-bottom:1.5rem;">
        ${PLAYOFF_DIVS.map(div => `
          <div style="margin-bottom:14px;">
            <div style="font-size:15px;font-weight:600;margin-bottom:6px;">${esc(div)}</div>
            ${(state.playoffFixtures[div]||[]).length ? (state.playoffFixtures[div]||[]).map((f,i) => `
              <div style="display:flex;justify-content:space-between;align-items:center;padding:6px 0;border-bottom:1px solid #2a2a2a;font-size:15px;">
                ...
                <span data-remove-playofffixture="${esc(div)}|${i}" style="cursor:pointer;color:#9a9a9a;font-size:14px;">remove</span>
              </div>`).join('') : `<p style="color:#9a9a9a;font-size:14px;">No fixtures set for this division.</p>`}
            <div style="display:flex;gap:6px;align-items:flex-end;margin-top:8px;flex-wrap:wrap;">
              <div style="min-width:160px;"><span style="font-size:13px;color:#9a9a9a;display:block;">Stage</span>
              ...
              <button class="bb-btn" data-add-playofffixture="${esc(div)}" style="padding:8px 14px;font-size:14px;">Add</button>
              ${(state.playoffFixtures[div]||[]).length ? `<button class="bb-btn ghost" data-clear-playofffixtures="${esc(div)}" style="padding:8px 14px;font-size:14px;">Clear all</button>` : ''}
            </div>
          </div>`).join('')}
        <p style="font-size:14px;color:#9a9a9a;margin-top:4px;">Rounds 24-26 are the playoff weeks for Division 2/3 (no regular H2H fixtures those weeks) -- set matchups here once the bracket's known.</p>
      </div>
      <h3>ECL group draw</h3>
      <div class="bb-card" style="margin-bottom:1.5rem;">
        <p style="font-size:14px;color:#9a9a9a;margin-top:0;">12 confirmed qualifiers this season. Assign each to a group of 4 once the real draw's known -- betting for a group opens automatically once it has all 4.</p>
        ${['A','B','C'].map(g => `
          <div style="margin-bottom:10px;">
            <div style="font-size:15px;font-weight:600;margin-bottom:4px;">Group ${g} (${(state.eclGroups[g]||[]).length}/4)</div>
            ${(state.eclGroups[g]||[]).map(t => `
              <span style="display:inline-flex;align-items:center;gap:6px;background:#262626;border:1px solid #3d3d3d;border-radius:14px;padding:3px 10px;margin:2px 4px 2px 0;font-size:14px;">
        ...
        <div style="display:flex;gap:6px;align-items:flex-end;margin-top:8px;flex-wrap:wrap;">
          ...
          <button class="bb-btn" data-assign-eclgroup="A" style="padding:8px 14px;font-size:14px;">Add to A</button>
          <button class="bb-btn" data-assign-eclgroup="B" style="padding:8px 14px;font-size:14px;">Add to B</button>
          <button class="bb-btn" data-assign-eclgroup="C" style="padding:8px 14px;font-size:14px;">Add to C</button>
          <button class="bb-btn ghost" id="clear-ecl-groups-btn" style="padding:8px 14px;font-size:14px;">Clear all</button>
        </div>
      </div>
`;
    const BETS_HTML = `\n      <h3>Resolve outstanding bets</h3>
      <div class="bb-card" style="margin-bottom:1.5rem;">
        ${(() => {
          const resolvable = bets.filter(b => (b.status||'PENDING')==='PENDING' && b.selections.length===1 &&
            (() => { const r = getPickRound(b.selections[0].id); return r !== null && r <= state.currentRound; })());
          if(!resolvable.length) return `<p style="color:#9a9a9a;font-size:15px;margin:0;">Nothing waiting on a result for this round or earlier.</p>`;
          return `<p style="color:#9a9a9a;font-size:14px;margin-top:0;">Single-selection bets on Round ${state.currentRound} or earlier, still pending. Multi-leg bets aren't shown here -- resolve those individually in the table below once every leg's known.</p>` +
            resolvable.map(b => `
              <div style="display:flex;justify-content:space-between;align-items:center;padding:8px 0;border-bottom:1px solid #2a2a2a;font-size:15px;">
                ...
                <span style="display:flex;gap:4px;">
                  <button class="bb-btn ghost" data-setstatus="${b.id}|WON" style="padding:4px 8px;font-size:13px;">Won</button>
                  <button class="bb-btn ghost" data-setstatus="${b.id}|LOST" style="padding:4px 8px;font-size:13px;">Lost</button>
                </span>
              </div>`).join('');
        })()}
      </div>
      ...
      <button class="bb-btn ghost" id="export-bets-csv" style="margin-bottom:10px;padding:6px 12px;font-size:14px;">Export to CSV</button>
      ${(() => {
        const readyCount = bets.filter(b => (b.status||'PENDING')==='PENDING' &&
          b.selections.some((s,i) => (b.selections.length===1 || !s.result) && computeSuggestedResult(s.id))).length;
        return readyCount ? `<p style="color:#ffdd00;font-size:14px;margin-bottom:8px;">\u26a1 ${readyCount} bet(s) below have a real result available and are ready to review -- highlighted first.</p>` : '';
      })()}
      <div class="bb-card" style="padding:0;overflow-x:auto;">
        <table class="bb-table">
            ...
            ${bets.slice().sort((a,b) => {
              ...
              <tr${betReady ? ' style="background:#3a3210;"' : ''}>
                ...
                <td>${b.selections.map((s,i)=>{
                  ...
                  const suggestionTag = suggestion ? ` <span style="color:#ffdd00;font-size:12px;font-weight:600;">&#9889; suggested: ${suggestion}</span>` : '';
                  ...
                  return `<div style="margin-bottom:4px;">${label} ${statusPill(legStatus)}${suggestionTag}<br/>
                    <span style="display:inline-flex;gap:3px;margin-top:2px;">
                      <span data-resolveleg="${b.id}|${i}|WON" style="cursor:pointer;color:#4a9166;font-size:12px;text-decoration:underline;${suggestion==='WON'?'font-weight:700;':''}">won</span>
                      <span data-resolveleg="${b.id}|${i}|LOST" style="cursor:pointer;color:#a3402f;font-size:12px;text-decoration:underline;${suggestion==='LOST'?'font-weight:700;':''}">lost</span>
                      <span data-resolveleg="${b.id}|${i}|VOID" style="cursor:pointer;color:#9a9a9a;font-size:12px;text-decoration:underline;">void</span>
                    </span></div>`;
                ...
                <td style="display:flex;gap:4px;flex-wrap:wrap;">
                  <button class="bb-btn ghost" data-setstatus="${b.id}|WON" style="padding:4px 8px;font-size:13px;${b.selections.length===1&&computeSuggestedResult(b.selections[0].id)==='WON'?'border-color:#ffdd00;':''}">Won</button>
                  <button class="bb-btn ghost" data-setstatus="${b.id}|LOST" style="padding:4px 8px;font-size:13px;${b.selections.length===1&&computeSuggestedResult(b.selections[0].id)==='LOST'?'border-color:#ffdd00;':''}">Lost</button>
                  <button class="bb-btn ghost" data-setstatus="${b.id}|VOID" style="padding:4px 8px;font-size:13px;">Kick (void)</button>
                  <button class="bb-btn ghost" data-setstatus="${b.id}|PENDING" style="padding:4px 8px;font-size:13px;">Reset</button>
                </td>
              </tr>`;
            }).join('')}
          </tbody>
    ...
    const SPECIALS_HTML = `\n      <h3>Specials &amp; Novelty</h3>
      <div class="bb-card" style="margin-bottom:1.5rem;">
        <div style="display:flex;gap:8px;align-items:flex-end;flex-wrap:wrap;margin-bottom:14px;">
          <div style="flex:2;min-width:200px;"><span style="font-size:14px;color:#9a9a9a;display:block;margin-bottom:4px;">Bet name</span>
            <input class="bb-input" id="novelty-name" placeholder="e.g. Someone forgets to make a trade all season"/></div>
          <div style="width:110px;"><span style="font-size:14px;color:#9a9a9a;display:block;margin-bottom:4px;">Odds</span>
        ...
        ${!(state.novelty||[]).length ? '<p style="color:#9a9a9a;font-size:15px;">Nothing added yet.</p>' : (state.novelty||[]).slice().sort((a,b)=>b.createdAt-a.createdAt).map(n => `
          <div style="padding:8px 0;border-bottom:1px solid #333333;">
            ${state.editingNoveltyId === n.id ? `
              <div style="display:flex;gap:8px;align-items:flex-end;flex-wrap:wrap;">
                <div style="flex:2;min-width:180px;"><span style="font-size:13px;color:#9a9a9a;display:block;">Bet name</span>
                  <input class="bb-input" id="edit-novelty-name-${n.id}" value="${esc(n.name)}"/></div>
                <div style="width:100px;"><span style="font-size:13px;color:#9a9a9a;display:block;">Odds</span>
                  <input class="bb-input" id="edit-novelty-odds-${n.id}" type="number" step="0.01" min="1.01" value="${n.odds}"/></div>
                <button class="bb-btn" data-save-novelty="${n.id}" style="padding:6px 10px;font-size:14px;">Save</button>
                <button class="bb-btn ghost" data-cancel-novelty-edit style="padding:6px 10px;font-size:14px;">Cancel</button>
              </div>
            ` : `
              <div style="display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap;">
                ...
                <span style="display:flex;gap:4px;flex-wrap:wrap;">
                  ${n.status==='OPEN' ? `
                    <button class="bb-btn ghost" data-noveltystatus="${n.id}|WON" style="padding:4px 8px;font-size:13px;">Won</button>
                    <button class="bb-btn ghost" data-noveltystatus="${n.id}|LOST" style="padding:4px 8px;font-size:13px;">Lost</button>
                    <button class="bb-btn ghost" data-noveltystatus="${n.id}|VOID" style="padding:4px 8px;font-size:13px;">Close (void)</button>
                  ` : `<button class="bb-btn ghost" data-noveltystatus="${n.id}|OPEN" style="padding:4px 8px;font-size:13px;">Reopen</button>`}
                  <button class="bb-btn ghost" data-edit-novelty="${n.id}" style="padding:4px 8px;font-size:13px;">Edit</button>
                  <button class="bb-btn ghost" data-delete-novelty="${n.id}" style="padding:4px 8px;font-size:13px;border-color:#a3402f;color:#c0604f;">Remove</button>
                </span>
              </div>
            `}
          </div>`).join('')}
      ...
      <div class="bb-card" style="margin-bottom:1.5rem;">
        ${(() => {
          ...
          if(!pendingSuggestions.length) return `<p style="color:#9a9a9a;font-size:15px;margin:0;">No ideas waiting on review.</p>`;
          return pendingSuggestions.map(s => `
            <div style="padding:8px 0;border-bottom:1px solid #333333;">
              <div style="font-size:15px;margin-bottom:6px;">${esc(s.text)} <span style="color:#8a8a8a;font-size:14px;">(from ${esc(s.submittedBy)})</span></div>
              <div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap;">
                ...
                <button class="bb-btn" data-approve-suggestion="${s.id}" style="padding:5px 10px;font-size:14px;">Approve &amp; add</button>
                <button class="bb-btn ghost" data-reject-suggestion="${s.id}" style="padding:5px 10px;font-size:14px;">Reject</button>
              </div>
            </div>`).join('');
        })()}
      </div>
`;
    const PUNTERS_HTML = `\n      <h3>Pending registrations</h3>
      ${!pending.length ? '<p style="color:#9a9a9a;font-size:15px;">Nothing waiting on approval.</p>' : `
      <div class="bb-card" style="padding:0;overflow:hidden;margin-bottom:1.5rem;">
        ${pending.length > 1 ? `<div style="padding:10px 14px;border-bottom:1px solid #3d3d3d;">
          <button class="bb-btn ghost" id="approve-all-btn" style="padding:5px 10px;font-size:14px;">Approve all ${pending.length}</button>
        </div>` : ''}
        ${pending.map((u,i) => {
          ...
          <div style="display:flex;justify-content:space-between;align-items:center;padding:10px 14px;${i<pending.length-1?'border-bottom:1px solid #3d3d3d;':''}">
            <span>${esc(u.username)} ${carry ? `<span style="color:#9a9a9a;font-size:14px;">(carry: ${fmt(carry)})</span>` : ''}</span>
            <span style="display:flex;gap:6px;">
              <button class="bb-btn" data-regstatus="${esc(u.username)}|APPROVED" style="padding:5px 10px;font-size:14px;">Approve (fund ${fmt(totalOnApproval)})</button>
              <button class="bb-btn ghost" data-regstatus="${esc(u.username)}|REJECTED" style="padding:5px 10px;font-size:14px;">Reject</button>
            </span>
          </div>`;
        }).join('')}
      </div>`}
            ...
            ${punters.slice().sort((a,b)=>a.username.localeCompare(b.username)).map(u => `
              <tr>
                <td>${esc(u.username)}
                  ...
                  ${u.status==='REJECTED' ? ` <span class="bb-pill" style="background:#f3ded9;color:#a3402f;">rejected</span> <span data-regstatus="${esc(u.username)}|APPROVED" style="cursor:pointer;color:#9a9a9a;font-size:13px;text-decoration:underline;">re-approve</span>` : ''}
                ...
                <td style="display:flex;gap:6px;align-items:center;">
                  ...
                  <button class="bb-btn ghost" data-adjust-user="${esc(u.username)}" style="padding:5px 10px;font-size:14px;">Apply</button>
                </td>
                <td style="display:flex;gap:4px;flex-wrap:wrap;">
                  ${u.isAdmin || u.status==='RESET' ? '' : (u.status==='KICKED'
                    ? `<button class="bb-btn ghost" data-regstatus="${esc(u.username)}|APPROVED" style="padding:5px 10px;font-size:14px;">Unkick</button>`
                    : `<button class="bb-btn ghost" data-kick-user="${esc(u.username)}" style="padding:5px 10px;font-size:14px;">Kick</button>`)}
                  ${u.isAdmin || u.status==='RESET' ? '' : `<button class="bb-btn ghost" data-reset-registration="${esc(u.username)}" style="padding:5px 10px;font-size:14px;">Reset registration</button>`}
                  ${u.isAdmin ? '' : `<button class="bb-btn ghost" data-delete-account="${esc(u.username)}" style="padding:5px 10px;font-size:14px;color:var(--bb-danger);border-color:var(--bb-danger);">Delete</button>`}
                </td>
              </tr>`).join('')}
          </tbody>
        </table>
...
`;
        ...
        <div class="bb-card">
          ${state.feedback.map(f => `<div style="padding:8px 0;border-bottom:1px solid #333333;">
              <div style="display:flex;justify-content:space-between;align-items:baseline;gap:8px;">
                <strong style="font-size:15px;">${esc(f.username)}</strong>
                <div style="display:flex;align-items:center;gap:8px;">
                  <span style="font-size:13px;color:#9a9a9a;white-space:nowrap;">${new Date(f.timestamp).toLocaleString()}</span>
                  <button class="bb-btn ghost" data-delete-feedback="${esc(f.id)}" style="padding:2px 8px;font-size:13px;">Remove</button>
                </div>
              </div>
              <div style="font-size:15px;color:#cfcfcf;margin-top:2px;">${esc(f.category)}</div>
              ${f.comment ? `<div style="font-size:14px;color:#9a9a9a;margin-top:4px;font-style:italic;">"${esc(f.comment)}"</div>` : ''}
            </div>`).join('')}
        </div>`;
    })();
  ...
  function renderEclGroupBox(group){
    ...
    let html = `<div class="bb-card" style="margin-bottom:14px;">
      <div style="font-size:16px;font-weight:600;margin-bottom:8px;">Group ${group}</div>`;
    if(!complete){
      html += `<p style="color:#9a9a9a;font-size:14px;">Not yet drawn &mdash; ${teams.length} of 4 teams assigned. Odds for this group will appear once the draw's confirmed.</p>`;
      if(teams.length){
        html += teams.map(t => `<div style="font-size:15px;padding:3px 0;">${esc(t)}</div>`).join('');
      }
    } else if(!state.roundBettingOpen && state.closeScope === 'all'){
      html += `<p style="color:#9a9a9a;font-size:14px;">\u{1F512} Betting is closed across all markets right now &mdash; hidden until it reopens.</p>`;
    } else {
    ...
    if(state.activeTab === 'HOME'){
      ...
      if(state.futuresSubTab === 'RODDY'){
        body = stripe + futuresSubTabBar() + roddyMarketTabs() + (state.futureMarketTab === 'leading_at'
          ...
          : `<button class="bb-btn ghost" id="surprise-me-btn" style="margin-bottom:10px;padding:6px 12px;font-size:14px;">\u{1F3B2} Surprise me</button>` + sectionRibbon() + `<div id="outcomes-list">${futuresOutcomesList('RODDY', state.futureMarketTab)}</div>`);
        ...
        body = stripe + futuresSubTabBar() + cupMarketTabs('fa_cup_labels') + sectionRibbon() +
          (state.futureMarketTab === 'fixtures' ? renderCupFixtures('FA CUP') :
            `<p style="color:#9a9a9a;font-size:14px;margin-bottom:10px;">Real Round of 64 draw from the 26/27 file: 62 entrants plus confirmed byes for Big Mac FC and Harvey Frekes. No matches played yet, so the whole bracket is simulated.</p>` +
        ...
        body = stripe + futuresSubTabBar() + cupMarketTabs('ecl_labels') + sectionRibbon() +
          (state.futureMarketTab === 'fixtures' ? renderCupFixtures('ECL') :
           state.futureMarketTab === 'groups' ? renderEclGroups() :
            `<p style="color:#9a9a9a;font-size:14px;margin-bottom:10px;">12 confirmed qualifiers for the 26/27 ECL, group draw not yet assigned (see the Groups tab). Stage odds below assume the field regardless of group -- they'll sharpen once groups are confirmed.</p>` +
        ...
        body = stripe + futuresSubTabBar() + futuresMarketTabs() + (state.futureMarketTab === 'leading_at'
          ...
          : `<button class="bb-btn ghost" id="surprise-me-btn" style="margin-bottom:10px;padding:6px 12px;font-size:14px;">\u{1F3B2} Surprise me</button>` + sectionRibbon() + `<div id="outcomes-list">${futuresOutcomesList(state.futuresSubTab, state.futureMarketTab)}</div>`);
      }
    } else {
  ...
  function slipBar(){
    if(!state.slip.length) return `<div class="bb-slip"><div class="bb-slip-inner" style="color:#9a9a9a;font-size:15px;">Tap any outcome to build a bet slip.</div></div>`;
    const modeToggle = `
      <div style="display:flex;gap:6px;margin-bottom:8px;">
        <div class="bb-tab ${state.betMode==='multi'?'active':''}" data-betmode="multi" style="font-size:14px;padding:5px 10px;">Multi (one combined bet)</div>
        <div class="bb-tab ${state.betMode==='singles'?'active':''}" data-betmode="singles" style="font-size:14px;padding:5px 10px;">Singles (bet each separately)</div>
      </div>`;
    const header = `<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
        <strong style="font-size:15px;">${state.slip.length} selection${state.slip.length>1?'s':''}</strong>
        <span style="display:flex;gap:6px;align-items:center;">
          ...
          <button class="bb-btn ghost" id="copy-slip" style="padding:4px 10px;font-size:14px;">Copy slip</button>
          <button class="bb-btn ghost" id="clear-slip" style="padding:4px 10px;font-size:14px;">Clear</button>
        </span>
      </div>`;
    ...
    if(state.betMode === 'singles'){
      ...
      return `<div class="bb-slip"><div class="bb-slip-inner">
        ...
        <div style="max-height:160px;overflow-y:auto;margin-bottom:8px;">
          ${state.slip.map(s => `<div style="display:flex;justify-content:space-between;align-items:center;gap:8px;font-size:14px;padding:5px 0;border-bottom:1px solid #333333;">
            ...
            <input class="bb-input" data-single-stake="${esc(s.id)}" type="number" min="1" value="${s.singleStake||50}" style="width:80px;padding:4px 6px;font-size:14px;"/>
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;">
          <span style="font-size:14px;color:#9a9a9a;">Total stake ${fmt(totalStake)} &rarr; potential ${fmt(totalPotential)}</span>
    ...
    const boostToggle = boostEligible ? `
      <label style="display:flex;align-items:center;gap:8px;font-size:14px;margin-bottom:8px;background:#4a3a10;border-radius:8px;padding:8px 10px;">
      ...
      <div style="font-size:13px;color:#9a9a9a;margin-bottom:8px;">This slip already includes a featured (boosted) pick, so the separate multi-boost can't also be applied.</div>` : '');
    return `<div class="bb-slip"><div class="bb-slip-inner">
      ${modeToggle}${header}
      <div style="max-height:100px;overflow-y:auto;margin-bottom:8px;">
        ${state.slip.map(s => `<div style="display:flex;justify-content:space-between;font-size:14px;padding:4px 0;border-bottom:1px solid #333333;">
      ...
      <div style="display:flex;gap:8px;align-items:center;">
        ...
        <div style="flex:1;"><span style="font-size:13px;color:#9a9a9a;">Combined odds</span>
          <div style="font-weight:600;color:#ffdd00;padding:6px 0;">${displayedCombined.toFixed(2)}${boostApplied?' \u26A1':''}${state.showImpliedChance?` <span style="font-size:13px;color:#9a9a9a;font-weight:400;">(${impliedChance(displayedCombined)})</span>`:''}</div></div>
  ...
  function renderHomeDigest(){
      ...
      if(kickoffStr){
        ...
        if(daysLeft > 0){
          roundPiece = `<div style="display:flex;align-items:center;gap:8px;padding:8px 0;">
              <span style="font-size:18px;">\u{1F4C5}</span>
              <span style="font-size:15px;">Round ${state.currentRound} kicks off in ${daysLeft} day${daysLeft!==1?'s':''}</span>
            </div>`;
        }
      }
    }
    ...
    const pendingPiece = pending.length
      ? `<div style="display:flex;align-items:center;gap:8px;padding:8px 0;">
          ...
          <span style="font-size:15px;">${pending.length} pending bet${pending.length!==1?'s':''} \u2014 <span style="color:#ffdd00;font-weight:600;">${fmt(pendingTotal)}</span> potential return</span>
        </div>` : '';

    const recentPiece = mostRecent
      ? `<div style="display:flex;align-items:center;gap:8px;padding:8px 0;">
          <span style="font-size:18px;">${mostRecent.status==='WON'?'\u2705':'\u274C'}</span>
          <span style="font-size:15px;">Your last bet ${mostRecent.status==='WON'?'won':'lost'}${mostRecent.status==='WON'?` \u2014 <span style="color:#ffdd00;font-weight:600;">${fmt(mostRecent.potentialReturn)}</span>`:''}</span>
        </div>` : '';
  ...
  function renderTippingNudgeCard(){
    ...
    return `<div class="bb-card" data-tab="TIPPING" style="margin-bottom:16px;cursor:pointer;display:flex;align-items:center;gap:10px;">
        ...
        <div><div style="font-weight:600;font-size:15px;">${esc(msg)}</div><div style="font-size:13px;color:#9a9a9a;margin-top:2px;">Tap to open Tipping \u2192</div></div>
      </div>`;
  }
  ...
  function renderAllTipstersTable(round, section){
    ...
    return `<div class="bb-card" style="margin-top:1rem;">
        <strong style="font-size:15px;">Everyone's tips \u2014 ${esc(section.label)}</strong>
        <div style="margin-top:8px;">
