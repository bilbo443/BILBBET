...
## Newer frontend audit scope (2026-09-23 handoff)

This tracker was completed against the older `app.js`. The priority
`files (1).zip/app.js` adds 21 named functions and changes substantial
rendering and handler code. Earlier counts (68 clickable attributes and 268
functions) are historical observations, not coverage claims for this version.

- [ ] Re-run mechanical handler/render matching on the priority frontend.
- [ ] Click through election markets and settlement review, median and
      unknown-opponent markets, Mr Median picks, fixture paste/import,
      install modal, countdown and remembered-username login/logout.
- [ ] Repeat mobile visual checks on the new content and confirm the app
      shell and icon paths in the assembled repository.

No new interaction tests were run while updating this documentation.
