...
## Scope of this review after the priority frontend handoff (2026-09-23)

This report records the earlier coefficient and odds rebuild; its tested
numbers belong to that snapshot. `files (1).zip` provides newer versions of
`app.js`, several H2H signals and histories, division membership and the
A-League projection. The earlier report does not establish that those newer
files were regenerated or deployed together. See `README.md` for the archive
precedence and `PRESEASON_TESTING.md` for outstanding integration checks.
