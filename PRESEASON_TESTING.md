...
## Newer frontend handoff and open checks (2026-09-23)

The additions below are present in the priority `files (1).zip` frontend.
They have been inspected in source; this handoff has **not** verified a live
site, browser interactions, Supabase records, or a completed deployment.
Keep the original Phase 4 and Phase 5 checkboxes open until actually run.

- [ ] Assemble the priority frontend under its referenced repo paths and
      verify all 16 `data/*.json` inputs are supplied by the combined
      project. The priority archive by itself is incomplete.
- [ ] Test election market visibility, placement and settlement against
      the actual election state. The source contains fixed results for 17
...
- [ ] Check fixture paste/import error handling, round countdown/timezone,
      ...
      handles same-origin GET requests; review what is cached in practice.
...
- [x] Build synthetic Division 3A/3B/3C schedules with 12, 13, 16 and 20 teams per conference. All four sizes pass, odd conferences use one AVERAGE TEAM fixture per regular round, and MW24–26 stay empty in the regular schedule.
- [x] Do not flag a conference as missing finals fixtures merely because it has none entered for one of MW24–26; its bracket may not be known or it may have no match that week. The admin fixture form remains available.
- [x] Finals H2H result suggestions use the entered Playoffs fixture for its exact round, even if the finalists met during the regular season. No suggestion is made for an unentered Division 2/3 fixture in a no-regular-fixture week.
- [ ] Test finals entry, display, and settlement in the Playoffs tab for their actual matchweek, including Division 3C when it exists.
...
- [x] While the sheet author has not added `26/27 DIVISION`, compare stable IDs and current names from the published All Time Data sheet with `admin_teams.json`, and verify the saved active roster against `h2h_divisions.json`. New IDs are reported as waitlist candidates; no old-season status is used to admit or place a team. The missing new-season column remains a sweep issue until the author provides it.
